...
import { getCurrentUserIdAction } from '@/lib/actions/auth' // <-- NUEVO IMPORT
...
export default function CrearProductoPage() {
  ...
  // ============================================================
  // CARGAR DATOS INICIALES (tipos + usuario actual)
  // ============================================================
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // 2. Obtener usuario actual mediante Server Action
  ...
  // ============================================================
  // VERIFICAR BUCKET DE SUPABASE STORAGE
  // ============================================================
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Validar usuario identificado
      if (!usuarioId) {
        toast.error('Error: No se pudo identificar al usuario. Vuelve a iniciar sesión.')
        setLoading(false)
        return
      }
      ...
      // 4. Crear producto (incluyendo id_usuario)
      const { data: producto, error: productError } = await supabase
        ...
        .insert({
          ...
          estado: true,
          id_usuario: usuarioId, // <-- Usuario creador
        })
        .select()
        .single() as any
      ...
      if (imagenes.length > 0) {
        ...
        if (urls.length > 0) {
          const { error: fotosError } = await supabase
            .from('fotos')
            .insert(
              urls.map((url) => ({ id_producto: producto.id_producto, foto: url }))
            ) as any
        ...
        if (erroresSubida > 0 && urls.length === 0) {
          ...
          toast.warning(
            `Producto creado con ${urls.length} imágenes. ${erroresSubida} imagen(es) fallaron.`
          )
        }
      }

      // 6. Registrar movimiento en la tabla registros
      await registrarMovimiento(
        producto.id_producto,
        `Creación del producto: ${producto.nombre_producto} (${producto.cantidad} unidades, Bs ${producto.precio.toFixed(2)})`
      )
